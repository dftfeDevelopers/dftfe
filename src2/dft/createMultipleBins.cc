			     ...
			     
{
	  ...
	  if(nodesIntersection.size() > 0)
	      ...
	      else if(iAtom < numberGlobalAtoms && jAtom >= numberGlobalAtoms)
		{
		  ...
		  if(masterAtomId == iAtom)
		    {
		      const std::string message("Atom and its own image is interacting decrease radius");
		      throw dft::IOError(message);
		    }
	      ...
	      else if(iAtom >= numberGlobalAtoms && jAtom >= numberGlobalAtoms)
		{
		  ...
		  if(masteriAtomId == masterjAtomId)
		    {
		      const std::string message("Two Image Atoms corresponding to same parent Atoms are interacting decrease radius");
		      throw dft::IOError(message);
		    }
  ...
  for(int iBin = 0; iBin < numberBins; ++iBin)
    {
      std::set<int> & atomsInBinSet = bins[iBin];
