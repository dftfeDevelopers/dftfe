...
{\it Description:} [Standard] Parameter to control verbosity of terminal output. 0 for low, 1 for medium, and 2 for high.
...
{\it Description:} [Standard] Boolean Parameter specifying whether pseudopotential DFT calculation needs to be performed.
...
{\it Description:} [Standard] Type of nonlocal projector to be used: 1 for KB, 2 for ONCV, default is KB.
...
{\it Description:} [Standard] Spin polarization: 0 for no spin polarization and 1 for spin polarization.
...
{\it Description:} [Developer] Boolean parameter which controls precomputation of FEM shape function gradients for each cell. This helps optimize discrete Hamiltonian matrix computation but at the cost of carrying some extra memory. Default value is false.
...
{\it Description:} [Developer] External mesh file path. If nothing is given auto mesh generation is performed
...
{\it Description:} [Standard] The degree of the finite-element interpolating polynomial
...
{\it Description:} [Developer] Radius of ball enclosing atom.
...
{\it Description:} [Developer] Mesh size of the base mesh on which refinement is performed.
...
{\it Description:} [Developer] Mesh size in a ball around atom.
...
{\it Description:} [Developer] Mesh size near atom. Useful for all-electron case.
...
\subsection{Parameters in section \tt Poisson problem paramters}
\label{parameters:Poisson_20problem_20paramters}

\begin{itemize}
\item {\it Parameter name:} {\tt MAXIMUM ITERATIONS}
\phantomsection\label{parameters:Poisson problem paramters/MAXIMUM ITERATIONS}
\label{parameters:Poisson_20problem_20paramters/MAXIMUM_20ITERATIONS}


\index[prmindex]{MAXIMUM ITERATIONS}
\index[prmindexfull]{Poisson problem paramters!MAXIMUM ITERATIONS}
{\it Value:} 10000
...
\phantomsection\label{parameters:Poisson problem paramters/P REFINEMENT}
\label{parameters:Poisson_20problem_20paramters/P_20REFINEMENT}


\index[prmindex]{P REFINEMENT}
\index[prmindexfull]{Poisson problem paramters!P REFINEMENT}
{\it Value:} false
...
\phantomsection\label{parameters:Poisson problem paramters/TOLERANCE}
\label{parameters:Poisson_20problem_20paramters/TOLERANCE}


\index[prmindex]{TOLERANCE}
\index[prmindexfull]{Poisson problem paramters!TOLERANCE}
{\it Value:} 1e-12
...
{\it Description:} [Developer] Boolean parameter specifying whether to use gemm\_batch blas routines to perform matrix-matrix multiplication operations with groups of matrices, processing a number of groups at once using threads instead of the standard serial route. CAUTION: batch blas routines will only be activated if the CHEBYSHEV FILTER BLOCK SIZE is less than 1000.
...
{\it Description:} [Developer] The degree of the Chebyshev polynomial to be employed for filtering out the unwanted spectrum (Default value is used when the input parameter value is 0.
...
{\it Description:} [Standard] Parameter specifying the type of orthogonalization to be used: GS(Gram-Schmidt Orthogonalization), LW(Lowden Orthogonalization), PGS(Pseudo Gram-Schmidt Orthogonalization). GS is the default option.
...
{\it Description:} [Developer] Sets the number of OpenMP threads to be used in the blas linear algebra calls inside Lowden Orthogonalization and Rayleigh-Ritz projection steps. The default value is 0, for which no action is taken. CAUTION: For non-zero values, CHEBYSHEV FILTER NUM OMP THREADS takes precedence over the OMP\_NUM\_THREADS environment variable.
