...
{
  std::vector<double>
  XmlTagReaderMain(std::vector<std::string> tag_name, std::string file_path_in)
  {
    xmlDocPtr  doc;
    xmlNodePtr cur;
  ...
  int
  xmlNodeChildCount(std::vector<std::string> tag_name, std::string file_path_in)
  {
    xmlDocPtr  doc;
    xmlNodePtr cur;
    doc = xmlParseFile(file_path_in.c_str());
    ...
    if (file.is_open())
      {
        for (int l = 0; l < radial_coord.size(); l++)
          {
            if (l == 0)
              file << radial_coord[0] << " " << rhoatom_values[0] << std::endl;
            else
              file << radial_coord[l] << " "
                   << rhoatom_values[l] /
    ...
    for (int i = 1; i <= xmlNodeChildCount(pswfc_tag, file_path_in); i++)
      {
        ...
        std::fstream file;
        file.open(file_path_out + "/" + attr_value[6] + ".dat", std::ios::out);
        file << std::setprecision(12);
        if (file.is_open())
          {
