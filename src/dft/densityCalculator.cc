	...
	template<unsigned int FEOrder,unsigned int FEOrderElectro>
		...
		(const std::vector<std::vector<dataTypes::number> > & eigenVectorsInput,
		 ...
		 const bool lobattoNodesFlag)
		 {
			 ...
			 const unsigned int numQuadPoints=lobattoNodesFlag?  psiEvalGL.n_q_points: psiEval.n_q_points;
			 ...
			 dealii::Tensor<1,2,dealii::Tensor<1,3,dealii::VectorizedArray<double> > > zeroTensor2;
			 ...
			 std::vector<std::vector<distributedCPUVec<double>>> eigenVectorsRotFrac((1+dftParameters::spinPolarized)*kPointWeights.size());
			 std::vector<distributedCPUVec<dataTypes::number> > eigenVectorsRotFracFlattenedBlock((1+dftParameters::spinPolarized)*kPointWeights.size());
			 ...
			 for(unsigned int ivec = 0; ivec < numEigenVectorsTotal; ivec+=eigenVectorsBlockSize)
			 {
				 ...
				 {
					 for(unsigned int kPoint = 0; kPoint < (1+dftParameters::spinPolarized)*kPointWeights.size(); ++kPoint)
					 {
						 ...
						 vectorTools::createDealiiVector<dataTypes::number>(mfData.get_vector_partitioner(0),
								 currentBlockSize,
								 eigenVectorsFlattenedBlock[kPoint]);
						 eigenVectorsFlattenedBlock[kPoint] = dataTypes::number(0.0);
					 }

					 constraintsNoneDataInfo.precomputeMaps(mfData.get_vector_partitioner(0),
				 ...
				 if (isRotFracEigenVectorsInBlock)
				 {
					 ...
					 if (currentBlockSizeFrac!=eigenVectorsRotFrac[0].size() || eigenVectorsRotFrac[0].size()==0)
					 {
						 ...
						 constraintsNoneDataInfo2.precomputeMaps(mfData.get_vector_partitioner(0),
						 ...
						 for(unsigned int iNode = 0; iNode < localVectorSize; ++iNode)
							 for(unsigned int iWave = 0; iWave < currentBlockSize; ++iWave)
								 eigenVectorsFlattenedBlock[kPoint].local_element(iNode*currentBlockSize+iWave)
									 = eigenVectorsInput[kPoint][iNode*numEigenVectorsTotal+ivec+iWave];
							 ...
							 for(unsigned int iNode = 0; iNode < localVectorSize; ++iNode)
								 for(unsigned int iWave = 0; iWave < currentBlockSizeFrac; ++iWave)
									 eigenVectorsRotFracFlattenedBlock[kPoint].local_element(iNode*currentBlockSizeFrac
											 +iWave)
										 = eigenVectorsFracInput[kPoint][iNode*numEigenVectorsFrac
...
#ifdef USE_COMPLEX
					 std::vector<dealii::Tensor<1,2,dealii::VectorizedArray<double> > > psiQuads(numQuadPoints*currentBlockSize*numKPoints,zeroTensor1);
					 std::vector<dealii::Tensor<1,2,dealii::VectorizedArray<double> > > psiQuads2(numQuadPoints*currentBlockSize*numKPoints,zeroTensor1);
					 std::vector<dealii::Tensor<1,2,dealii::Tensor<1,3,dealii::VectorizedArray<double> > > > gradPsiQuads(numQuadPoints*currentBlockSize*numKPoints,zeroTensor2);
					 std::vector<dealii::Tensor<1,2,dealii::Tensor<1,3,dealii::VectorizedArray<double> > > > gradPsiQuads2(numQuadPoints*currentBlockSize*numKPoints,zeroTensor2);

					 std::vector<dealii::Tensor<1,2,dealii::VectorizedArray<double> > > psiRotFracQuads(numQuadPoints*currentBlockSizeFrac*numKPoints,zeroTensor1);
					 std::vector<dealii::Tensor<1,2,dealii::VectorizedArray<double> > > psiRotFracQuads2(numQuadPoints*currentBlockSizeFrac*numKPoints,zeroTensor1);
					 std::vector<dealii::Tensor<1,2,dealii::Tensor<1,3,dealii::VectorizedArray<double> > > > gradPsiRotFracQuads(numQuadPoints*currentBlockSizeFrac*numKPoints,zeroTensor2);
					 std::vector<dealii::Tensor<1,2,dealii::Tensor<1,3,dealii::VectorizedArray<double> > > > gradPsiRotFracQuads2(numQuadPoints*currentBlockSizeFrac*numKPoints,zeroTensor2);
#else
					 std::vector< dealii::VectorizedArray<double> > psiQuads(numQuadPoints*currentBlockSize,dealii::make_vectorized_array(0.0));
					 std::vector< dealii::VectorizedArray<double> > psiQuads2(numQuadPoints*currentBlockSize,dealii::make_vectorized_array(0.0));
					 std::vector< dealii::Tensor<1,3,dealii::VectorizedArray<double> > > gradPsiQuads(numQuadPoints*currentBlockSize,zeroTensor3);
					 std::vector< dealii::Tensor<1,3,dealii::VectorizedArray<double> > > gradPsiQuads2(numQuadPoints*currentBlockSize,zeroTensor3);

					 std::vector< dealii::VectorizedArray<double> > psiRotFracQuads(numQuadPoints*currentBlockSizeFrac,dealii::make_vectorized_array(0.0));
					 std::vector< dealii::VectorizedArray<double> > psiRotFracQuads2(numQuadPoints*currentBlockSizeFrac,dealii::make_vectorized_array(0.0));
					 std::vector<dealii::Tensor<1,3,dealii::VectorizedArray<double> > > gradPsiRotFracQuads(numQuadPoints*currentBlockSizeFrac,zeroTensor3);
					 std::vector<dealii::Tensor<1,3,dealii::VectorizedArray<double> > > gradPsiRotFracQuads2(numQuadPoints*currentBlockSizeFrac,zeroTensor3);
#endif

					 for (unsigned int cell=0; cell<mfData.n_macro_cells(); ++cell)
					 {

						 lobattoNodesFlag?psiEvalGL.reinit(cell):psiEval.reinit(cell);

						 const unsigned int numSubCells=mfData.n_components_filled(cell);
						 ...
						 for(unsigned int kPoint = 0; kPoint < numKPoints; ++kPoint)
							 for(unsigned int iEigenVec=0; iEigenVec<currentBlockSize; ++iEigenVec)
							 {
								 ...
								 lobattoNodesFlag?
									 psiEvalGL.read_dof_values_plain(eigenVectors[(1+dftParameters::spinPolarized)*kPoint][iEigenVec]):
									 psiEval.read_dof_values_plain(eigenVectors[(1+dftParameters::spinPolarized)*kPoint][iEigenVec]);

								 if(isEvaluateGradRho)
									 lobattoNodesFlag?psiEvalGL.evaluate(true,true):psiEval.evaluate(true,true);
								 else
									 lobattoNodesFlag?psiEvalGL.evaluate(true,false):psiEval.evaluate(true,false);

								 for (unsigned int q=0; q<numQuadPoints; ++q)
								 {
									 psiQuads[q*currentBlockSize*numKPoints+currentBlockSize*kPoint+iEigenVec]=lobattoNodesFlag?psiEvalGL.get_value(q):psiEval.get_value(q);
									 if(isEvaluateGradRho)
										 gradPsiQuads[q*currentBlockSize*numKPoints+currentBlockSize*kPoint+iEigenVec]=lobattoNodesFlag?psiEvalGL.get_gradient(q):psiEval.get_gradient(q);
								 }

								 if(dftParameters::spinPolarized==1)
								 {

									 lobattoNodesFlag?psiEvalGL.read_dof_values_plain
									 ...
									 for (unsigned int q=0; q<numQuadPoints; ++q)
									 {
										 psiQuads2[q*currentBlockSize*numKPoints+currentBlockSize*kPoint+iEigenVec]=lobattoNodesFlag?psiEvalGL.get_value(q):psiEval.get_value(q);
										 if(isEvaluateGradRho)
											 gradPsiQuads2[q*currentBlockSize*numKPoints+currentBlockSize*kPoint+iEigenVec]= lobattoNodesFlag?psiEvalGL.get_gradient(q):psiEval.get_gradient(q);
									 }
								 }
								 ...
								 if (isRotFracEigenVectorsInBlock && iEigenVec>=startingIndexFrac)
								 {
									 ...
									 lobattoNodesFlag?psiEvalGL.read_dof_values_plain(eigenVectorsRotFrac[(1+dftParameters::spinPolarized)*kPoint][vectorIndex]):psiEval.read_dof_values_plain(eigenVectorsRotFrac[(1+dftParameters::spinPolarized)*kPoint][vectorIndex]);
									 ...
									 for (unsigned int q=0; q<numQuadPoints; ++q)
									 {
										 psiRotFracQuads[q*currentBlockSizeFrac*numKPoints
											 +currentBlockSizeFrac*kPoint+vectorIndex]=lobattoNodesFlag?psiEvalGL.get_value(q):psiEval.get_value(q);
										 if(isEvaluateGradRho)
											 gradPsiRotFracQuads[q*currentBlockSizeFrac*numKPoints
												 +currentBlockSizeFrac*kPoint+vectorIndex]=lobattoNodesFlag?psiEvalGL.get_gradient(q):psiEval.get_gradient(q);
									 }

									 if(dftParameters::spinPolarized==1)
									 {

										 lobattoNodesFlag?psiEvalGL.read_dof_values_plain(eigenVectorsRotFrac[(1+dftParameters::spinPolarized)*kPoint+1][vectorIndex]):psiEval.read_dof_values_plain(eigenVectorsRotFrac[(1+dftParameters::spinPolarized)*kPoint+1][vectorIndex]);
							 ...
							 for(unsigned int kPoint = 0; kPoint < numKPoints; ++kPoint)
							 {
								 for(unsigned int iEigenVec=0; iEigenVec<currentBlockSize; ++iEigenVec)
								 {
									 ...
									 for(unsigned int q=0; q<numQuadPoints; ++q)
									 {
										 ...
										 std::vector<dealii::Tensor<1,3,double> > gradPsi(2),gradPsi2(2);
										 ...
										 if (isRotFracEigenVectorsInBlock && iEigenVec>=startingIndexFrac)
										 {
...
#ifdef USE_COMPLEX
											 dealii::Vector<double> psiRotFrac, psiRotFrac2;
											 ...
											 std::vector<dealii::Tensor<1,3,double> > gradPsiRotFrac(2),gradPsiRotFrac2(2);
											 ...
											 dealii::Tensor<1,3,double> gradPsiRotFrac,gradPsiRotFrac2;
											 if(isEvaluateGradRho)
												 for(unsigned int idim=0; idim<3; ++idim)
												 {
													 gradPsiRotFrac[idim]
											 ...
											 {
												 rhoTemp[q] += 2.0*kPointWeights[kPoint]
													 *(partialOccupancy*(psiRotFrac(0)*psiRotFrac(0)
																 + psiRotFrac(1)*psiRotFrac(1))
															 -(psiRotFrac(0)*psiRotFrac(0)
																 + psiRotFrac(1)*psiRotFrac(1))
										 ...
										 else
										 {
											 ...
											 if(dftParameters::spinPolarized==1)
											 {
												 rhoTempSpinPolarized[2*q] += partialOccupancy*kPointWeights[kPoint]*(psi(0)*psi(0) + psi(1)*psi(1));
												 rhoTempSpinPolarized[2*q+1] += partialOccupancy2*kPointWeights[kPoint]*(psi2(0)*psi2(0) + psi2(1)*psi2(1));
												 //
												 if(isEvaluateGradRho)
													 for(unsigned int idim=0; idim<3; ++idim)
													 {
														 gradRhoTempSpinPolarized[6*q + idim] +=
															 2.0*partialOccupancy*kPointWeights[kPoint]*(psi(0)*gradPsi[0][idim] + psi(1)*gradPsi[1][idim]);
														 gradRhoTempSpinPolarized[6*q + 3+idim] +=
															 2.0*partialOccupancy2*kPointWeights[kPoint]*(psi2(0)*gradPsi2[0][idim] + psi2(1)*gradPsi2[1][idim]);
													 }
											 }
											 else
											 {
												 rhoTemp[q] += 2.0*partialOccupancy*kPointWeights[kPoint]*(psi(0)*psi(0) + psi(1)*psi(1));
												 if(isEvaluateGradRho)
													 for(unsigned int idim=0; idim<3; ++idim)
														 gradRhoTemp[3*q + idim] += 2.0*2.0*partialOccupancy*kPointWeights[kPoint]*(psi(0)*gradPsi[0][idim] + psi(1)*gradPsi[1][idim]);
											 }
#else
											 if(dftParameters::spinPolarized==1)
											 {
