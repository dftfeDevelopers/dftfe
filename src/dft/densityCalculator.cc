...
namespace dftfe
{
    ...
    const bool           spectrumSplit)
  {
    ...
    if (this_process == 0 && dftParams.verbosity >= 2)
      if (memorySpace == dftfe::utils::MemorySpace::HOST)
        std::cout << "Time for compute tau on CPU: " << computeTau_time
                  << std::endl;
      else if (memorySpace == dftfe::utils::MemorySpace::DEVICE)
        std::cout << "Time for compute tau on Device: " << computeTau_time
                  << std::endl;
  }
      ...
      for (unsigned int iQuad = 0; iQuad < nQuadsPerCell; ++iQuad)
        for (unsigned int iWave = 0; iWave < vecRange.second - vecRange.first;
             ++iWave)
          {
            ...
            tau[iCell * nQuadsPerCell + iQuad] += 0.5 * partialOccupVec[iWave] *
                                                  kPointCoordSq *
                                                  std::abs(psi) * std::abs(psi);

            tau[iCell * nQuadsPerCell + iQuad] +=
              partialOccupVec[iWave] *
              dftfe::utils::imagPart(tempImag * dftfe::utils::complexConj(psi));
          }
  }
