...
namespace dftfe
{
  ...
  // For energyCalculator for the multiplekpoints in TauMGGA XC
  template <typename NumberType, dftfe::utils::MemorySpace memorySpace>
  void
  computeExtraTermsForEnergyCalcTauMgga(
    ...
    std::vector<
      dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST>>
      &kSquareWeightedWfcSquareValues,
    std::vector<
      dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST>>
    ...
    const bool           spectrumSplit)
  {
    ...
    MPI_Comm_rank(mpiCommParent, &this_process);
#if defined(DFTFE_WITH_DEVICE)
    std::string errMsg = "TauMGGA for device Not implemented yet.";
    dftfe::utils::throwException(false, errMsg);
    if (memorySpace == dftfe::utils::MemorySpace::DEVICE)
      dftfe::utils::deviceSynchronize();
#endif
    MPI_Barrier(mpiCommParent);
    double             computeExtraTermsTauMggaEnergyCalc_time = MPI_Wtime();
    const unsigned int numKPoints   = kPointWeights.size();
    const unsigned int numLocalDofs = basisOperationsPtr->nOwnedDofs();
    const unsigned int totalLocallyOwnedCells = basisOperationsPtr->nCells();
    const unsigned int numNodesPerElement = basisOperationsPtr->nDofsPerCell();
    ...
    dftfe::utils::MemoryStorage<NumberType, memorySpace> gradWfcQuadPointData;
    dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST>
      kSquareWeightedWfcSquareHost;
    dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST>
      conjUKGradUMinusUKConjGradUHost;
    ...
    dftfe::utils::MemoryStorage<double, memorySpace>
      conjUKGradUMinusUKConjGradUHost;
#else
    auto &kSquareWeightedWfcSquare    = kSquareWeightedWfcSquareHost;
    auto &conjUKGradUMinusUKConjGradU = conjUKGradUMinusUKConjGradUHost;
#endif

    kSquareWeightedWfcSquare.resize(totalLocallyOwnedCells * numQuadPoints *
                                      numSpinComponents,
                                    0.0);
    conjUKGradUMinusUKConjGradU.resize(totalLocallyOwnedCells * numQuadPoints *
                                         numSpinComponents,
                                       0.0);
    wfcQuadPointData.resize(cellsBlockSize * numQuadPoints * BVec, zero);
                ...
                if ((jvec + currentBlockSize) <=
                      ...
                      bandGroupLowHighPlusOneIndices[2 * bandGroupTaskId])
                  {
                    if (spectrumSplit)
                      {
                        std::string errMsg =
                          "spectrumSplit Not implemented in SCAN yet.";
                        dftfe::utils::throwException(false, errMsg);
                      }
                    else
                      {
                        ...
                        if (currentCellsBlockSize > 0)
                          {
                            ...
                            computeExtraTermsTauMGGAFromInterpolatedValues(
                              ...
                              wfcQuadPointData.data(),
                              gradWfcQuadPointData.data(),
                              kSquareWeightedWfcSquare.data() +
                                spinIndex * totalLocallyOwnedCells *
                                  numQuadPoints,
                              conjUKGradUMinusUKConjGradU.data() +
                                spinIndex * totalLocallyOwnedCells *
                                  numQuadPoints,
                              kCoord.data());
    ...
    std::string errMsg = "Device implementation is not yet there in SCAN.";
    dftfe::utils::throwException(false, errMsg);
    ...
    if (size > 1)
      {
        MPI_Allreduce(
          MPI_IN_PLACE,
          kSquareWeightedWfcSquareHost.data(),
          totalLocallyOwnedCells * numQuadPoints * numSpinComponents,
          dataTypes::mpi_type_id(kSquareWeightedWfcSquareHost.data()),
          MPI_SUM,
          interpoolcomm);

        MPI_Allreduce(
          MPI_IN_PLACE,
          conjUKGradUMinusUKConjGradUHost.data(),
          totalLocallyOwnedCells * numQuadPoints * numSpinComponents,
          dataTypes::mpi_type_id(conjUKGradUMinusUKConjGradUHost.data()),
          MPI_SUM,
          interpoolcomm);
      }
    ...
    if (dftParams.spinPolarized == 1)
      {
        kSquareWeightedWfcSquareValues[0].resize(totalLocallyOwnedCells *
                                                 numQuadPoints);
        kSquareWeightedWfcSquareValues[1].resize(totalLocallyOwnedCells *
                                                 numQuadPoints);

        std::transform(kSquareWeightedWfcSquareHost.begin(),
                       kSquareWeightedWfcSquareHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       kSquareWeightedWfcSquareHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       kSquareWeightedWfcSquareValues[0].begin(),
                       std::plus<>{});
        ...
        std::transform(kSquareWeightedWfcSquareHost.begin(),
                       ...
                       kSquareWeightedWfcSquareHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       kSquareWeightedWfcSquareValues[1].begin(),
                       std::minus<>{});


        conjUKGradUMinusUKConjGradUValues[0].resize(totalLocallyOwnedCells *
                                                    numQuadPoints);
        conjUKGradUMinusUKConjGradUValues[1].resize(totalLocallyOwnedCells *
                                                    numQuadPoints);

        std::transform(conjUKGradUMinusUKConjGradUHost.begin(),
                       conjUKGradUMinusUKConjGradUHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       conjUKGradUMinusUKConjGradUHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       conjUKGradUMinusUKConjGradUValues[0].begin(),
                       std::plus<>{});
        ...
        std::transform(conjUKGradUMinusUKConjGradUHost.begin(),
                       ...
                       conjUKGradUMinusUKConjGradUHost.begin() +
                         totalLocallyOwnedCells * numQuadPoints,
                       conjUKGradUMinusUKConjGradUValues[1].begin(),
                       std::minus<>{});
      }
    else
      {
        kSquareWeightedWfcSquareValues[0]    = kSquareWeightedWfcSquareHost;
        conjUKGradUMinusUKConjGradUValues[0] = conjUKGradUMinusUKConjGradUHost;
      }
#if defined(DFTFE_WITH_DEVICE)
    if (memorySpace == dftfe::utils::MemorySpace::DEVICE)
      dftfe::utils::deviceSynchronize();
#endif
    MPI_Barrier(mpiCommParent);
    computeExtraTermsTauMggaEnergyCalc_time =
      MPI_Wtime() - computeExtraTermsTauMggaEnergyCalc_time;

    if (this_process == 0 && dftParams.verbosity >= 2)
      if (memorySpace == dftfe::utils::MemorySpace::HOST)
        std::cout
          << "Time for Extra terms copmutation for Energy in TauMGGA on CPU: "
          << computeExtraTermsTauMggaEnergyCalc_time << "s" << std::endl;
    // else if (memorySpace == dftfe::utils::MemorySpace::DEVICE)
    //   std::cout << "Time for compute rho on Device: " << computeRho_time
    //             << std::endl;
  }
             ...
             ++iWave)
          {
            const NumberType psi =
              wfcQuadPointData[(iCell - cellRange.first) * nQuadsPerCell *
                               ...
                               iQuad * vectorsBlockSize + iWave];
            double halfKSquare =
              0.5 * (kcoord[0] * kcoord[0] + kcoord[1] * kcoord[1] +
                     kcoord[2] * kcoord[2]);
            kSquareWeightedWfcSquare[iCell * nQuadsPerCell + iQuad] +=
              partialOccupVec[iWave] * halfKSquare * std::abs(psi) *
              std::abs(psi);

            NumberType tempVar = 0.0;
            ...
            for (unsigned int iDim = 0; iDim < 3; iDim++)
              {
                tempVar +=
                  ...
                  gradWfcQuadPointData[(iCell - cellRange.first) *
                                         nQuadsPerCell * vectorsBlockSize * 3 +
                                       iDim * nQuadsPerCell * vectorsBlockSize +
                                       iQuad * vectorsBlockSize + iWave];
              }
            conjUKGradUMinusUKConjGradU[iCell * nQuadsPerCell + iQuad] =
              partialOccupVec[iWave] *
              dftfe::utils::imagPart(dftfe::utils::complexConj(psi) * tempVar);
          }
  }
