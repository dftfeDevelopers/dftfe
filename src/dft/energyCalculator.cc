	...
	namespace internal {
				...
				const unsigned int verbosity)
		{
			...
			if (reproducibleOutput)
			{
				...
				pcout << "-------------------"<<std::endl;
				if (dftParameters::useMixedPrecPGS_O || dftParameters::useMixedPrecPGS_SR || dftParameters::useMixedPrecCheby)
		...
		double nuclearElectrostaticEnergyLocal(const distributedCPUVec<double> & phiTotRhoOut,
				...
				const bool smearedNuclearCharges=false)
		{
			...
			else
			{
				...
				for (; cell!=endc; ++cell) 
					...
					{
						const std::vector<double> & bQuadValuesCell=smearedbValues.find(cell->id())->second;
	...
	double energyCalculator::computeEnergy
     ...
     const std::map<dealii::CellId, std::vector<double> > & gradRhoCoreValues,       
		 ...
		 const bool smearedNuclearCharges) const
		 {
			 ...
			 dealii::FEValues<3> feValuesElectronicLpsp (dofHandlerElectronic.get_fe(), quadratureLpsp, dealii::update_JxW_values);

			 dealii::FEValues<3> feValuesElectrostaticLpsp (dofHandlerElectrostatic.get_fe(), quadratureLpsp, dealii::update_JxW_values);
       ...
       if (rhoOutValuesElectrostaticLpsp.size()!=0)
       {
         ...
         AssertThrow( num_quad_points_lpsp == rhoOutValuesElectrostaticLpsp.begin()->second.size(),
           ...
           feValuesElectronicLpsp.reinit(cellElectronic);
						 ...
						 if(dftParameters::nonLinearCoreCorrection == true)
             {
               for (unsigned int q_point=0; q_point<num_quad_points_electronic; ++q_point)
               {
                 ...
                 const double gradRhoOutX = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 0])+(gradRhoCoreValues.find(cellElectronic->id())->second[3*q_point + 0]);
                 const double gradRhoOutY = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 1])+(gradRhoCoreValues.find(cellElectronic->id())->second[3*q_point + 1]);
                 const double gradRhoOutZ = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 2])+(gradRhoCoreValues.find(cellElectronic->id())->second[3*q_point + 2]);
                 const double gradValRhoOutX = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 0]);
                 const double gradValRhoOutY = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 1]);
                 const double gradValRhoOutZ = (gradRhoOutValues.find(cellElectronic->id())->second[3*q_point + 2]);
           ...
           if(dftParameters::isPseudopotential || smearedNuclearCharges)
           {
             ...
             for (unsigned int q_point = 0; q_point < num_quad_points_lpsp; ++q_point)
                 electrostaticPotentialTimesRho+=tempPspCorr[q_point]*tempRho[q_point]*feValuesElectronicLpsp.JxW (q_point);
           }
           ...
           if(dftParameters::isPseudopotential || smearedNuclearCharges)
           {
             ...
             for (unsigned int q_point = 0; q_point < num_quad_points_lpsp; ++q_point)
                 electrostaticEnergyTotPot+=tempPspCorr[q_point]*tempRho[q_point]*feValuesElectrostaticLpsp.JxW (q_point);
           }           
				 }
