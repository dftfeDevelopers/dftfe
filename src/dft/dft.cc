...
namespace dftfe
{
  ...
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  dftClass<FEOrder, FEOrderElectro>::writeGSElectronDensity(const std::string Path) const
  {
    ...
    if (poolId == 0 && bandGroupId == 0)
      {
        ...
        for (; cell != endc; ++cell)
            {
              if (cell->is_locally_owned())
                {
                  fe_values.reinit(cell);
                  const std::vector<double> & rhoValues   = (d_dftParamsPtr->spinPolarized == 1)?rhoOutValuesSpinPolarized->find(cell->id())->second:rhoOutValues->find(cell->id())->second;
                  ...
                  for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
                    {
                      ...
                      data.push_back(std::make_shared<dftUtils::QuadDataCompositeWrite>(quadVals));
                    }
                }
            }
        ...
        dftUtils::MPIWriteOnFile().writeData(dataRawPtrs, Path, mpi_communicator);
