					  ...
					  const bool lobattoNodesFlag)
{
  ...
  const unsigned int localVectorSize = d_eigenVectorsFlattenedSTL[0].size()/numEigenVectorsTotal;
   ...
   std::vector<std::vector<distributedCPUVec<double>>> eigenVectorsRotFrac((1+dftParameters::spinPolarized)*d_kPointWeights.size());
   std::vector<distributedCPUVec<dataTypes::number> > eigenVectorsRotFracFlattenedBlock((1+dftParameters::spinPolarized)*d_kPointWeights.size());
