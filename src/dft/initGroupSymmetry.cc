...
//
 ...
 //dofHandlerEigen_serial(triangulation_serial) ;
 //dofHandlerEigen_serial.distribute_dofs(FEEigen_serial);
 ...
 //
  ...
  for (; cell!=endc; ++cell) 
    ...
    if (cell->is_locally_owned()) {
		...
		rhoRecvd[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<double>>(n_mpi_processes) ;
	  }
	  ...
	  for (unsigned int iSymm = 0; iSymm < numSymm; ++iSymm) {
	     ...
	     //
	     send_buf_size[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<int>>(n_mpi_processes);
	     //
	     mappedGroupSend0[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<int> >(n_mpi_processes);
	     mappedGroupSend2[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<int> >(n_mpi_processes);
	     mappedGroupSend1[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<std::vector<double>> >(n_mpi_processes);
	     //
		 ...
		 mappedGroupSend1[iSymm][globalCellId_parallel[cell->id()]][i] = std::vector<std::vector<double>>(3);
             ...
             recv_buf_size[iSymm][globalCellId_parallel[cell->id()]][0] = std::vector<int>(n_mpi_processes);
	     recv_buf_size[iSymm][globalCellId_parallel[cell->id()]][1] = std::vector<int>(n_mpi_processes);
	     recv_buf_size[iSymm][globalCellId_parallel[cell->id()]][2] = std::vector<int>(n_mpi_processes);
	     //
	     groupOffsets[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<int>>(3);
	     groupOffsets[iSymm][globalCellId_parallel[cell->id()]][0] = std::vector<int>(n_mpi_processes);
	     groupOffsets[iSymm][globalCellId_parallel[cell->id()]][1] = std::vector<int>(n_mpi_processes);
	     groupOffsets[iSymm][globalCellId_parallel[cell->id()]][2] = std::vector<int>(n_mpi_processes);
             //
	     //
	     //if (cell->is_locally_owned()) {
   ...
   for (; cell!=endc; ++cell) 
     ...
     for (unsigned int iSymm=0; iSymm<numSymm; ++iSymm)
	{
	 ...
	 for (unsigned int proc=0; proc<n_mpi_processes; ++proc){
		...
		recv_size [proc] += (1+spinPolarized)*send_buf_size[iSymm][globalCellId_parallel[cell->id()]][proc][1] ;
   ...
   for (; cell!=endc; ++cell) 
    {
     ...
     for (unsigned int iSymm=0; iSymm<numSymm; ++iSymm)
	{
	 gradRhoRecvd[iSymm][globalCellId_parallel[cell->id()]] = std::vector<std::vector<double>>(n_mpi_processes) ;
	 for (unsigned int proc=0; proc<n_mpi_processes; ++proc){
