    ...
    &_pseudoValuesAtoms)
{
  ...
  for (unsigned int localDofId = 0; localDofId < phiExt.local_size();
       ++localDofId)
    {
      if (localDofId < kptGroupLowHighPlusOneIndicesStep1[2 * kptGroupTaskId + 1] &&
  ...
  for (unsigned int macrocell = 0;
       ...
       ++macrocell)
    {
      ...
      if (macrocell < kptGroupLowHighPlusOneIndicesStep2[2 * kptGroupTaskId + 1] &&
  ...
  {
    std::vector<double> tempPseudoValuesFlattened(numberElements*n_q_points,0.0);
    ...
    for (; cell != endc; ++cell)
        if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
             tempPseudoValuesFlattened[elemId*n_q_points+q]=pseudoVLoc[q];
        }
    ...
    for (; cell != endc; ++cell)
        if (cell->is_locally_owned())
        {
          ...
          std::vector<double> &pseudoVLoc = _pseudoValues[cell->id()];            
          for (unsigned int q = 0; q < n_q_points; ++q)
            pseudoVLoc[q]=tempPseudoValuesFlattened[elemId*n_q_points+q];
        }
  }
      ...
      if (cell->is_locally_owned())
        {
          ...
          if (ielem < kptGroupLowHighPlusOneIndicesStep3[2 * kptGroupTaskId + 1] &&
  ...
  if (numberElements>0 && numberKptGroups>1)
  {
        ...
        for (; cell != endc; ++cell)
            if (cell->is_locally_owned())
            {
              if (_pseudoValuesAtoms.find(iAtom)!=_pseudoValuesAtoms.end())
                if (_pseudoValuesAtoms[iAtom].find(cell->id())!=_pseudoValuesAtoms[iAtom].end())
                {
                  sendCount++;
                  pseudoVLocAtom=_pseudoValuesAtoms[iAtom][cell->id()];
                  sendData.push_back(iAtom);
                  sendData.push_back(ielem);
                  sendData.insert(sendData.end(),pseudoVLocAtom.begin(),pseudoVLocAtom.end());
                }
              ielem++;
    ...
    ierr =
      MPI_Allgatherv(sendCount > 0 ? &sendData[0] :
                                        &dummy,
                     sendCount,
                     MPI_DOUBLE,
                     &recvData[0],
    ...
    {
       const unsigned int iatom=std::round(recvData[i* (2+n_q_points)+0]);
       const unsigned int elementId=std::round(recvData[i* (2+n_q_points)+1]);

       for (unsigned int q = 0; q < n_q_points; ++q)
          pseudoVLocAtom[q]=recvData[i* (2+n_q_points)+1+q];

       _pseudoValuesAtoms[iatom][elemIdToCellIdMap[elementId]] = pseudoVLocAtom;
    }
