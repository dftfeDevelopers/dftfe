...
namespace dftfe
{
  ...
  void expConfiningPotential::init(const std::shared_ptr<
                        ...
                        const std::vector<std::vector<double>>& atomLocations)
  {
    ...
    for (unsigned int iAtom = 0 ;iAtom < atomLocations.size(); iAtom++)
      {
        double atomDist = 0.0;
        atomDist = atomLocations[iAtom][2]*atomLocations[iAtom][2]*periodicFactorX;
        atomDist += atomLocations[iAtom][3]*atomLocations[iAtom][3]*periodicFactorY;
        atomDist += atomLocations[iAtom][4]*atomLocations[iAtom][4]*periodicFactorZ;
        if ( atomDist < maxDist)
          {
            maxDist = atomDist;
          }
    ...
    for (unsigned int iQuad = 0 ; iQuad < numQuadPoints; iQuad++)
      {
        double quadDist = 0.0;
        quadDist = quadPoints[3*iQuad + 0]*quadPoints[3*iQuad + 0]*periodicFactorX;
        quadDist += quadPoints[3*iQuad + 1]*quadPoints[3*iQuad + 1]*periodicFactorY;
        ...
        if ( quadDist < maxDist + r1)
        ...
        else
          {
            ...
            d_confiningPotential.data()[iQuad] = dftParams.confiningCParam*expFactor/(tol*tol);
          }
      }
  }

  void expConfiningPotential::addConfiningPotential(dftfe::utils::MemoryStorage<double, dftfe::utils::MemorySpace::HOST>& externalPotential) const
  {
