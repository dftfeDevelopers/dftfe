...
namespace dftfe
{
  ...
  )
  {
    ...
    //
    // project and create a nodal field of the same mesh from the quadrature
    // data (L2 projection from quad points to nodes)
    //
    distributedCPUVec<double> rhoNodalFieldCoarse;
    d_matrixFreeDataPRefined.initialize_dof_vector(
      rhoNodalFieldCoarse, d_baseDofHandlerIndexElectro);
    ...
    //
    // subdivide the existing mesh and project electron-density onto the new
    // mesh
    //
    ...
    if (!d_dftParamsPtr->floatingNuclearCharges)
      {
        //
        // move the refined mesh so that it forms exact subdivison of coarse
        // moved mesh
        //
    ...
    dealii::DoFHandler<3>::active_cell_iterator cell = dofHandlerHRefined
                                                         .begin_active(),
    ...
    dealii::QIterated<3> quadraturelpsp(
      dealii::QGauss<1>(C_num1DQuadLPSP<FEOrder>()), C_numCopies1DQuadLPSP());
      ...
      if (cell->is_locally_owned())
        {
          ...
          fe_values_lpspquad.get_function_values(d_rhoNodalFieldRefined,
                                                 rholpsp);
    ...
    if (d_dftParamsPtr->constraintsParallelCheck)
      {
        ...
        dealii::DoFTools::extract_locally_active_dofs(
          dofHandlerHRefined, locally_active_dofs_debug);

        const std::vector<dealii::IndexSet> &locally_owned_dofs_debug =
          dealii::Utilities::MPI::all_gather(
            mpi_communicator, dofHandlerHRefined.locally_owned_dofs());

        AssertThrow(
          constraintsHRefined.is_consistent_in_parallel(
        ...
        for (unsigned int i = 2; i < matrixFreeConstraintsInputVector.size();
             i++)
          AssertThrow(
    ...
    const double totalEnergy = d_dftParamsPtr->spinPolarized == 0 ?
                                 energyCalcHRefined.computeEnergy(
                                   dofHandlerHRefined,
                                   dofHandler,
                                   quadrature,
                                   quadrature,
