...
void dftClass::init(){
  ...
  for(std::set<unsigned int>::iterator iter = masterNodes.begin(); iter != masterNodes.end(); ++iter)
    {
  ...
  std::cout<<"Size of MasterNodes List : "<<masterNodes.size()<<std::endl;
  std::cout<<"Size of MasterNodes Really containing on its processor: "<<masterNodeCount<<std::endl;*/
  ...
  for(types::global_dof_index i = 0; i <dofHandlerEigen.n_dofs(); ++i){
	...
	if (constraintsTempEigen.is_identity_constrained(i)){
	  ...
	  if (count==1){
	    ...
	    if (!((std::abs(p[l]-q[l])<periodicPrecision) and (std::abs(p[m]-q[m])<periodicPrecision))){
	      bool foundNewMaster=false;
	      for (std::set<unsigned int>::iterator it=masterNodesEigen.begin(); it!=masterNodesEigen.end(); ++it){
