...
namespace dftfe
{
    ...
    dealiiLinearSolver &CGSolver)
  {
    ...
    d_densityInNodalValues[0].add(const2, d_preCondResidualVector);
    ...
    dealiiLinearSolver &CGSolver)
  {
    ...
    //     // initialize data structures for computing mixing constants in
    //     Anderson
    //     // mixing scheme
    ...
    //     dgesv_(&N, &NRHS, &ATotal[0], &lda, &ipiv[0], &cTotal[0], &ldb,
    //     &info); if ((info > 0) && (this_mpi_process == 0))
    //       {
    //         printf(
    //           "Anderson Mixing: The diagonal element of the triangular factor
    //           of A,\n");
    //         printf(
    ...
    //     gradDensityResidualValuesMap; for (unsigned int cell = 0;
    //          cell < d_matrixFreeDataPRefined.n_cell_batches();
    //          ++cell)
    //       {
    //         fe_evalHelm.reinit(cell);
    ...
    //     // initialize helmholtz solver function object with the quantity
    //     required
    //     // for computing rhs, solution vector and mixing constant

    //     if (d_dftParamsPtr->verbosity >= 2)
    //       pcout
    //         << "Solving Helmholtz equation for Kerker Preconditioning of
    //         nodal fields: "
    //         << std::endl;
    ...
    //     double const2      = -d_dftParamsPtr->mixingParameter;
    //     d_rhoInNodalValues.add(1.0, rhoInBar, const2,
    ...
    //   gradRhoInValues->find(subCellId)->second; for(unsigned int q_point = 0;
    //   q_point < numQuadPoints; ++q_point)
    //   {
    //   tempVec[3*q_point + 0] = fe_evalRho.get_gradient(q_point)[0][iSubCell];
    //   tempVec[3*q_point + 1] = fe_evalRho.get_gradient(q_point)[1][iSubCell];
    //   tempVec[3*q_point + 2] = fe_evalRho.get_gradient(q_point)[2][iSubCell];
