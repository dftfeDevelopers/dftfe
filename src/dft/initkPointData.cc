...
//============================================================================================================================================
//============================================================================================================================================
//    This is the source file for generating Monkhorst-Pack Brillouin zone (BZ) grid and creating the irreducible BZ
//	                             Only relevant for calculations using multiple k-points
//
...
//============================================================================================================================================
//============================================================================================================================================
//			          Following routine computes the reciprocal lattice vectors for the given system
//============================================================================================================================================
//============================================================================================================================================ 
std::vector<std::vector<double> > getReciprocalLatticeVectors(const std::vector<std::vector<double> > & latticeVectors, const std::array<unsigned int,3> periodicity)
  {
  ...
  for(unsigned int i = 0; i < 2; ++i)
    {
    cross=internaldft::cross_product(latticeVectors[i+1], latticeVectors[3 - (2*i + 1)]);
    scalarConst = latticeVectors[i][0]*cross[0] + latticeVectors[i][1]*cross[1] + latticeVectors[i][2]*cross[2];
    for (unsigned int d = 0; d < 3; ++d)
	reciprocalLatticeVectors[i][d] = (2.*M_PI/scalarConst)*cross[d];
    }
  //
  cross=internaldft::cross_product(latticeVectors[0],latticeVectors[1]);
  scalarConst = latticeVectors[2][0]*cross[0] + latticeVectors[2][1]*cross[1] + latticeVectors[2][2]*cross[2];
  for (unsigned int d = 0; d < 3; ++d)
  ...
  for (unsigned int d = 0; d < 3; ++d)			                               
      unitVectorOutOfPlane[d] = cross [d] / ( sqrt ( cross[0]*cross[0] + cross[1]*cross[1] + cross[2]*cross[2] ) ) ;
  //
  for(unsigned int i = 0; i < 2; ++i)
     {
  ...
  for(unsigned int i = 0; i < 3; ++i)
     ...
     if (periodicity[i]==1)
        {
        const double scalarConst = sqrt ( latticeVectors [i][0]*latticeVectors [i][0] + latticeVectors [i][1]*latticeVectors [i][1] + latticeVectors [i][2]*latticeVectors [i][2] ) ;
	for (unsigned int d = 0; d < 3; ++d)
              reciprocalLatticeVectors[i][d] = (2.*M_PI/scalarConst )*latticeVectors[i][d];
        }
     }
  } // end switch
  //
...
//============================================================================================================================================
//============================================================================================================================================
//			           Following routine can read k-points supplied through external file
...
void dftClass<FEOrder>::readkPointData()
{
  ...
  sprintf(kPointRuleFile, "%s/data/kPointList/%s", DFT_PATH, dftParameters::kPointDataFile.c_str());
...
//============================================================================================================================================
//============================================================================================================================================
//			           Main driver routine to generate the MP grid, reduce BZ using point group symmetries 
     ...
     while( ik < nk ) 
        ...
        for (unsigned int iSymm = 1; iSymm < symmetryPtr->numSymm; ++iSymm) // iSymm begins from 1. because identity is always present and is taken care of.
	   {
	      ...
	      while (kPointTemp[dir] < -1.0E-5 )
           ...
           //
           unsigned int ikx = ( round(kPointTemp[0]*(1+offsetFlagX)*nkx) - offsetFlagX ) ;
           unsigned int iky = ( round(kPointTemp[1]*(1+offsetFlagY)*nky) - offsetFlagY ) ;
           unsigned int ikz = ( round(kPointTemp[2]*(1+offsetFlagZ)*nkz) - offsetFlagZ ) ;
           //
	   if (ikx%(1+offsetFlagX)==0)
               ikx = ( round(kPointTemp[0]*(1+offsetFlagX)*nkx) - offsetFlagX ) / (1+offsetFlagX) ;
	   else
	       ikx = 1001 ;
	   if (iky%(1+offsetFlagY)==0)
               iky = ( round(kPointTemp[1]*(1+offsetFlagY)*nky) - offsetFlagY ) / (1+offsetFlagY) ;
	   else
	       iky = 1001 ;
            if (ikz%(1+offsetFlagZ)==0)
               ikz = ( round(kPointTemp[2]*(1+offsetFlagZ)*nkz) - offsetFlagZ ) / (1+offsetFlagZ) ;
	   else
	       ikz = 1001 ;
           //
