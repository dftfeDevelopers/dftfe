          ...
          for (unsigned int iFace = 0; iFace < faces_per_cell; ++iFace)
            {
			  ...
			  if(!d_dftParamsPtr->smearedNuclearCharges)
			  {
				  ...
				  {
					  double dist = 0.0;
					  dist = (atomLocations[iAtom][2] - p[0])*(atomLocations[iAtom][2] - p[0]);
					  dist += (atomLocations[iAtom][3] - p[1])*(atomLocations[iAtom][3] - p[1]);
					  dist += (atomLocations[iAtom][4] - p[2])*(atomLocations[iAtom][4] - p[2]);
				          dist = std::sqrt(dist);
					  if(d_dftParamsPtr->isPseudopotential)
					  {
						  constraintValue -= atomLocations[iAtom][1] /dist;
					  }
					  else
					  {
						  constraintValue -= atomLocations[iAtom][0] /dist;
					  }
				  } 
			  }
