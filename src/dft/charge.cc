  ...
  const std::map<dealii::CellId, std::vector<double>> *rhoQuadValues, bool centerofCharge)
{
...

  {
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
            {
              dipolevector[0] += rhoValues[q_point] * fe_values.JxW(q_point)*fe_values.quadrature_point(q_point)[0];
              dipolevector[1] += rhoValues[q_point] * fe_values.JxW(q_point)*fe_values.quadrature_point(q_point)[1];
              dipolevector[2] += rhoValues[q_point] * fe_values.JxW(q_point)*fe_values.quadrature_point(q_point)[2];
            }
        }
    }
                      MPI_Allreduce(MPI_IN_PLACE,
                              ...
                              MPI_SUM,
                              MPI_COMM_WORLD);
    pcout<<"Electron dipole moment: "<<dipolevector[0]<<" "<<dipolevector[1]<<" "<<dipolevector[2]<<std::endl;
    ...
    pcout<<"Total dipole moment: "<<dipolevector[0]<<" "<<dipolevector[1]<<" "<<dipolevector[2]<<std::endl;
  }
  else
  {
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
            {
              dipolevector[0] += rhoValues[q_point] * fe_values.JxW(q_point)*(fe_values.quadrature_point(q_point)[0]-COM[0]);
              dipolevector[1] += rhoValues[q_point] * fe_values.JxW(q_point)*(fe_values.quadrature_point(q_point)[1]-COM[1]);
              dipolevector[2] += rhoValues[q_point] * fe_values.JxW(q_point)*(fe_values.quadrature_point(q_point)[2]-COM[2]);
            }
        }
    }
                    MPI_Allreduce(MPI_IN_PLACE,
                              ...
                              MPI_SUM,
                              MPI_COMM_WORLD);
    pcout<<"Electron dipole moment wrt COM: "<<dipolevector[0]<<" "<<dipolevector[1]<<" "<<dipolevector[2]<<std::endl;

    for(int iAtom = 0; iAtom < atomLocations.size(); iAtom++)
    {
      dipolevector[0] += -atomLocations[iAtom][1]*(atomLocations[iAtom][2]-COM[0]);
      dipolevector[1] += -atomLocations[iAtom][1]*(atomLocations[iAtom][3]-COM[1]);
      dipolevector[2] += -atomLocations[iAtom][1]*(atomLocations[iAtom][4] - COM[2]);

    }
    pcout<<"Total dipole moment wrt COM: "<<dipolevector[0]<<" "<<dipolevector[1]<<" "<<dipolevector[2]<<std::endl;
