      ...
      if (dftParameters::useELPA)
        {
          ...
          dftfe::ScaLAPACKMatrix<T> overlapMatParTrans(numberVectors,
                                                       processGrid,
                                                       rowsBlockSize);
...
#  endif
      ...
      dftfe::ScaLAPACKMatrix<T> LMatPar(numberVectors,
                                        processGrid,
                                        rowsBlockSize,
                                        overlapMatPropertyPostCholesky);
...
#  endif
                                       ...
                                       const bool         useMixedPrec)
    {
      ...
      // of column vectors and (Zc)^T is conjugate transpose of Z

      computing_timer.enter_section("local overlap matrix for pgs");

      // Since input "X" is stored as number of column vectors times number of
      // degrees of freedom matrix corresponding to column-major format required
      // for blas, we compute the transpose of overlap matrix i.e Sc =
      // X^{T}*(Xc) here
      const char   uplo1  = 'L';
      const char   trans1 = 'N';
...
#  endif
      ...
      computing_timer.enter_section(
        "PGS cholesky and triangular matrix invert");
...
#  ifdef USE_COMPLEX
      zpotrf_(
        &uplo2, &numberVectors, &overlapMatrix[0], &numberVectors, &info2);
...
#  endif
      ...
      // X=X*Lc^{-1}^{T} implemented as X^{T}=Lc^{-1}*X^{T} with X^{T} stored in
      // the column major format
