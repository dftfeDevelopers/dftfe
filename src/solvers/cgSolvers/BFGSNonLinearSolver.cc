...
namespace dftfe
{
  ...
  BFGSNonLinearSolver::computeLambda()
  {
    ...
    */
    ...
    double dxtHdx =
      ddot_(&d_numberUnknowns, d_deltaX.data(), &uone, Hdx.data(), &uone);
    ...
    double dxtSdx =
      ddot_(&d_numberUnknowns, d_deltaX.data(), &uone, Sdx.data(), &uone);

    double gtdx = ddot_(
      &d_numberUnknowns, d_deltaX.data(), &uone, d_gradient.data(), &uone);
