// ---------------------------------------------------------------------
//
// Copyright (c) 2018-2020 The Regents of the University of Michigan and DFT-FE authors.
//
...
namespace dftfe {
	...
	//solve
	void cgLinearSolverCUDA::solve(linearSolverProblemCUDA & problem,
			...
			const unsigned int  debugLevel)
	{
		...
		if(debugLevel >= 2)
		{
			...
			pcout<< "GPU based Linear Conjugate Gradient solver started with residual norm "<<std::sqrt(residualNorm)<<std::endl;
		...
		if(debugLevel >= 2)
		{
			if(solveStatus == 1)
			{
				pcout<< "Linear Conjugate Gradient solver converged after "
					<<iterationNumber+1 <<" iterations. with residual norm "<<std::sqrt(residualNorm)<<std::endl;
			}
			else
			{
				pcout<< "Linear Conjugate Gradient solver failed to converge after "
					<<iterationNumber <<" iterations. with residual norm "<<std::sqrt(residualNorm)<<std::endl;
			}
		}
		...
		if (debugLevel >= 2)
			std::cout<<"Time for Poisson problem iterations: "<<gpu_time<<std::endl;
