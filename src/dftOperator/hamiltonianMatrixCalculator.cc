  ...
  const unsigned int spinIndex)
{
  ...
  if ((dftParameters::isPseudopotential ||
      ...
      !d_isStiffnessMatrixExternalPotCorrComputed)
    {
      ...
      d_cellHamiltonianMatrixExternalPotCorr.resize(sizeNiNj *
                                                      totalLocallyOwnedCells,
                                                    0.0);


      std::vector<double> NiNjLpspQuad_currentBlock(numberEntriesEachBlock *
          ...
          for (unsigned int q_point = 0; q_point < numberQuadraturePoints;
               ++q_point)
            {
              ...
              for (unsigned int iNode =
                ...
                {
                  double shapeI =
                    d_shapeFunctionLpspQuadData[numberDofsPerElement * q_point +
                                                iNode];
                  for (unsigned int jNode =
  ...
  std::vector<double> cellHamiltonianMatrix(totalLocallyOwnedCells * sizeNiNj,
                                            0.0);
                  ...
                  for (unsigned int jNode =
                       ...
                       ++jNode)
                    {
                      ...
                      gradNiNjPlusgradNjNi_currentBlock
                        [3 * numberEntriesEachBlock * q_point + indexCount] =
                          shapeGradXRefINode * shapeJ +
                          shapeI * d_shapeFunctionGradientValueRefX
                      ...
                      gradNiNjPlusgradNjNi_currentBlock
                        [3 * numberEntriesEachBlock * q_point +
                         numberEntriesEachBlock + indexCount] =
                          ...
                          shapeI * d_shapeFunctionGradientValueRefY
                                     [numberDofsPerElement * q_point + jNode];
                      gradNiNjPlusgradNjNi_currentBlock
                        [3 * numberEntriesEachBlock * q_point +
                         2 * numberEntriesEachBlock + indexCount] =
                          shapeGradZRefINode * shapeJ +
                          shapeI * d_shapeFunctionGradientValueRefZ
                                     [numberDofsPerElement * q_point + jNode];
                      indexCount += 1;
                      if (indexCount % numberEntriesEachBlock == 0)
                        {
  ...
  double kSquareTimesHalf =
    0.5 * (kPointCoors[0] * kPointCoors[0] + kPointCoors[1] * kPointCoors[1] +
               ...
               ++iNode)
            {
              ...
              for (unsigned int jNode =
                   ...
                   ++jNode)
                {
                  double shapeJ =
                    d_shapeFunctionData[numberDofsPerElement * q_point + jNode];
                  gradNiNj_currentBlock[3 * numberEntriesEachBlock * q_point +
                                        indexCount] =
                    shapeGradXRefINode * shapeJ;
                  gradNiNj_currentBlock[3 * numberEntriesEachBlock * q_point +
                                        numberEntriesEachBlock + indexCount] =
