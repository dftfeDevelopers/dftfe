...
template<unsigned int FEOrder>
void kohnShamDFTOperatorClass<FEOrder>::computeNonLocalHamiltonianTimesXBatchGEMMSinglePrec
	     ...
	     dealii::parallel::distributed::Vector<dataTypes::number>  & dst) const
{
  ...
  std::map<unsigned int, std::vector<dataTypes::numberLowPrec> > projectorKetTimesVector;
  ...
  const char transA = 'N';
  const char transB = 'N';
  const dataTypes::numberLowPrec alpha = 1.0;
  const dataTypes::numberLowPrec beta = 1.0;
  const unsigned int inc = 1;
  std::vector<dataTypes::number> temp(numberWaveFunctions);
  std::vector<dataTypes::numberLowPrec> cellWaveFunctionMatrix(d_numberNodesPerElement*numberWaveFunctions,0.0);
  typename DoFHandler<3>::active_cell_iterator cell = dftPtr->dofHandler.begin_active(), endc = dftPtr->dofHandler.end();
  int iElem = -1;
  for(; cell!=endc; ++cell)
      if(cell->is_locally_owned())
	  ...
	  if (dftPtr->d_nonLocalAtomIdsInElement[iElem].size()>0)
	    for(unsigned int iNode = 0; iNode < d_numberNodesPerElement; ++iNode)
	    {
	      ...
	      for (unsigned int iwave=0; iwave< numberWaveFunctions;++iwave)
                   cellWaveFunctionMatrix[numberWaveFunctions*iNode+iwave]
	  ...
	  for(unsigned int iAtom = 0; iAtom < dftPtr->d_nonLocalAtomIdsInElement[iElem].size();++iAtom)
	    {
	      ...
	      sgemm_(&transA,
		     ...
		     &cellWaveFunctionMatrix[0],
		     &numberWaveFunctions,
		     &dftPtr->d_nonLocalProjectorElementMatricesLowPrec[atomId][nonZeroElementMatrixId][d_kPointIndex][0],
		     &d_numberNodesPerElement,
		     &beta,
		     &projectorKetTimesVector[atomId][0],
		     &numberWaveFunctions);
  ...
  dftPtr->d_projectorKetTimesVectorParFlattenedLowPrec=0.0;
  ...
  for(unsigned int iAtom = 0; iAtom < dftPtr->d_nonLocalAtomIdsInCurrentProcess.size(); ++iAtom)
    {
      ...
      for(unsigned int iPseudoAtomicWave = 0; iPseudoAtomicWave < numberPseudoWaveFunctions; ++iPseudoAtomicWave)
      {
          ...
          scopy_(&numberWaveFunctions,
                &projectorKetTimesVector[atomId][numberWaveFunctions*iPseudoAtomicWave],
                &inc,
                &dftPtr->d_projectorKetTimesVectorParFlattenedLowPrec[id*numberWaveFunctions],
                &inc);

      }
    }

  dftPtr->d_projectorKetTimesVectorParFlattenedLowPrec.compress(VectorOperation::add);
  dftPtr->d_projectorKetTimesVectorParFlattenedLowPrec.update_ghost_values();
  ...
  for(unsigned int iAtom = 0; iAtom < dftPtr->d_nonLocalAtomIdsInCurrentProcess.size(); ++iAtom)
    {
      ...
      for(unsigned int iPseudoAtomicWave = 0; iPseudoAtomicWave < numberPseudoWaveFunctions; ++iPseudoAtomicWave)
	{
	  dataTypes::numberLowPrec  nonlocalConstantV
	      =(dataTypes::numberLowPrec)dftPtr->d_nonLocalPseudoPotentialConstants[atomId][iPseudoAtomicWave];

          const unsigned int id=dftPtr->d_projectorIdsNumberingMapCurrentProcess[std::make_pair(atomId,iPseudoAtomicWave)];

	  sscal_(&numberWaveFunctions,
		 &nonlocalConstantV,
		 &dftPtr->d_projectorKetTimesVectorParFlattenedLowPrec[id*numberWaveFunctions],
		 &inc);
  ...
  const char transA1 = 'N';
  const char transB1 = 'N';
  const dataTypes::numberLowPrec alpha1 = 1.0;
  const dataTypes::number alpha1HighPrec=1.0;
  const dataTypes::numberLowPrec  beta1 = 0.0;
  const unsigned int inc1 = 1;
  ...
  for(unsigned int iAtom = 0; iAtom < dftPtr->d_nonLocalAtomIdsInCurrentProcess.size(); ++iAtom)
    {
       ...
       const unsigned int groupCount=1;
       const unsigned int groupSize=dftPtr->d_elementIteratorsInAtomCompactSupport[atomId].size();
       dataTypes::numberLowPrec ** cellProjectorKetTimesVectorMatrixBatch = new dataTypes::numberLowPrec*[groupSize];
       dataTypes::numberLowPrec ** cellNonLocalHamTimesWaveMatrixBatch = new dataTypes::numberLowPrec*[groupSize];
       const dataTypes::numberLowPrec ** cellNonLocalProjectorKetBatch = new dataTypes::numberLowPrec*[groupSize];
       for(unsigned int i = 0; i < groupSize; i++)
	    cellNonLocalHamTimesWaveMatrixBatch[i] = new dataTypes::numberLowPrec[d_numberNodesPerElement*numberWaveFunctions];
       ...
       for(unsigned int iElemComp = 0; iElemComp < dftPtr->d_elementIteratorsInAtomCompactSupport[atomId].size(); ++iElemComp)
	{
		...
		=&projectorKetTimesVector[atomId][0];
	    cellNonLocalProjectorKetBatch[iElemComp]
		=&dftPtr->d_nonLocalProjectorElementMatricesTransposeLowPrec[atomId][iElemComp][d_kPointIndex][0];
	}

        sgemm_batch_(&transA1,
		     &transB1,
		     &numberWaveFunctions,
		     &d_numberNodesPerElement,
		     &numberPseudoWaveFunctions,
	    ...
	    for(unsigned int iNode = 0; iNode < d_numberNodesPerElement; ++iNode)
	    {
	      ...
	      for (unsigned int iwave=0; iwave< numberWaveFunctions;++iwave)
		   temp[iwave]=(dataTypes::number)
		      cellNonLocalHamTimesWaveMatrixBatch[iElemComp][numberWaveFunctions*iNode+iwave];
