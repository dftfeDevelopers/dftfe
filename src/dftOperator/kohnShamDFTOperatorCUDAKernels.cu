...
namespace dftfe
{
  ...
  __global__ void
  daxpyBinCUDAKernel(
    const unsigned int                     innerContiguousBlockSize,
    ...
    const double                           a)
  {
    const unsigned int globalThreadId = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int numberEntries =
      numContiguousBlocks * innerContiguousBlockSize;

    for (unsigned int index = globalThreadId; index < numberEntries;
         index += blockDim.x * gridDim.x)
      {
        unsigned int blockIndex =
          index / innerContiguousBlockSize; // degree of freedom index
        unsigned int outerBlockIndex =
          blockIndex / outerContiguousBlockSize; // cell id
        unsigned int actualOuterBlockIndex = binContents[outerBlockIndex];
        unsigned int actualIndex           = actualOuterBlockIndex *
                                   outerContiguousBlockSize *
                                   innerContiguousBlockSize;
        unsigned int intraOuterBlockIndex =
          blockIndex % outerContiguousBlockSize;
        unsigned int intraInnerBlockIndex = index % innerContiguousBlockSize;
        yVec[yVecStartingContiguousBlockIds[blockIndex] +
             intraInnerBlockIndex] +=
          a *
          xVec[actualIndex + intraOuterBlockIndex * innerContiguousBlockSize +
               intraInnerBlockIndex];
      }
  }
    ...
    for (unsigned int index = globalThreadId; index < numberEntries;
         index += blockDim.x * gridDim.x)
      {
        ...
        const unsigned int flattenedId =
          addToVecStartingContiguousBlockIds[blockIndex];
        if (boundaryIdVec[flattenedId / contiguousBlockSize] == 0)
          atomicAdd(&addToVec[flattenedId + intraBlockIndex],
                    addFromVec[index]);
      }
  }
