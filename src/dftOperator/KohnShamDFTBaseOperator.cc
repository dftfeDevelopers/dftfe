...
namespace dftfe
{
    ...
    const dftfe::uInt spinIndex)
  {
    ...
    pdexDensitySpinUp.resize(numberQuadraturePointsPerCell *
                               totalLocallyOwnedCells,
                             0.0);
    pdecDensitySpinUp.resize(numberQuadraturePointsPerCell *
                               totalLocallyOwnedCells,
                             0.0);
    ...
    if (isGGA)
      {
        xDataOut[xcRemainderOutputDataAttributes::pdeSigma] =
          std::vector<double>(3 * numberQuadraturePointsPerCell *
                                totalLocallyOwnedCells,
                              0.0);
    ...
    if (isTauMGGA)
      {
        xDataOut[xcRemainderOutputDataAttributes::pdeTauSpinUp] =
          std::vector<double>(numberQuadraturePointsPerCell *
                                totalLocallyOwnedCells,
                              0.0);
    ...
    for (dftfe::uInt iCell = 0; iCell < totalLocallyOwnedCells; ++iCell)
      {
        ...
        for (dftfe::uInt iQuad = 0; iQuad < numberQuadraturePointsPerCell;
             ++iQuad)
          {
            dftfe::uInt offset = iCell * numberQuadraturePointsPerCell + iQuad;
            d_VeffJxWHost[iCell * numberQuadraturePointsPerCell + iQuad] =
              (tempPhi[iQuad] + pdexDensitySpinIndex[offset] +
               pdecDensitySpinIndex[offset]) *
              cellJxWPtr[iQuad];
          }
                ...
                for (dftfe::uInt iQuad = 0;
                     ...
                     ++iQuad)
                  {
                    ...
                    const double *gradDensityQuadPtr =
                      ...
                      iCell * 3 * numberQuadraturePointsPerCell;
                    const double *gradDensityOtherQuadPtr =
                      gradDensityXCOtherSpinIndex.data() + iQuad * 3 +
                      iCell * 3 * numberQuadraturePointsPerCell;
                    const double term =
                      (pdexSigma[offset * 6 + 2 * spinIndex] +
                       pdecSigma[offset * 6 + 2 * spinIndex]) *
                      cellJxWPtr[iQuad];
                    const double termoff =
                      (pdexSigma[offset * 6 + 1] + pdecSigma[offset * 6 + 1]) *
                      cellJxWPtr[iQuad];
                    for (dftfe::uInt jDim = 0; jDim < 3; ++jDim)
                      for (dftfe::uInt iDim = 0; iDim < 3; ++iDim)
                        d_invJacderExcWithSigmaTimesGradRhoJxWHost
                ...
                for (dftfe::uInt iQuad = 0;
                     ...
                     ++iQuad)
                  {
                    ...
                    const double *gradDensityOtherQuadPtr =
                      ...
                      iCell * 3 * numberQuadraturePointsPerCell;
                    const double term =
                      (pdexSigma[offset * 3 + 2 * spinIndex] +
                       pdecSigma[offset * 3 + 2 * spinIndex]) *
                      cellJxWPtr[iQuad];
                    const double termoff =
                      (pdexSigma[offset * 3 + 1] + pdecSigma[offset * 3 + 1]) *
                      cellJxWPtr[iQuad];
                    for (dftfe::uInt iDim = 0; iDim < 3; ++iDim)
                      d_invJacderExcWithSigmaTimesGradRhoJxWHost
                ...
                for (dftfe::uInt iQuad = 0;
                     ...
                     ++iQuad)
                  {
                    ...
                    const double termTau =
                      0.5 *
                      (pdexTauSpinIndex[offset] + pdecTauSpinIndex[offset]) *
                      cellJxWPtr[iQuad];
                            ...
                            for (dftfe::uInt iDim = 0; iDim < 3; ++iDim)
                              {
                                d_derExcwithTauTimesinvJacKpointTimesJxWHost
                                  [iCell * numberQuadraturePointsPerCell * 3 +
                                   iQuad * 3 + iDim + offsetFactor * 3] +=
                                  -0.5 *
                                  inverseJacobiansQuadPtr[3 * jDim + iDim] *
                                  kPointCoords[jDim] * cellJxWPtr[iQuad] *
                                  (pdexTauSpinIndex[offset] +
                                   pdecTauSpinIndex[offset]);
                              }
                          }
                      }
                  }
