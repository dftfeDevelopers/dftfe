...
namespace dftfe
{
    namespace
    {
	    ...
	    getKeyValuePairs(const std::string filename,
	...
	std::vector<std::string> keysToFind = {"PTC_FILE", "RHO_TOL", "S_THRESHOLD"};
	...
	for(unsigned int i = 0; i < keysToFind.size(); ++i)
	{
	  ...
	  utils::throwException(found, "Unable to find the key " + keysToFind[i] + " in file " + d_modelFilename);
	}
