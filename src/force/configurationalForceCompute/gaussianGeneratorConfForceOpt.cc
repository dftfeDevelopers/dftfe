...
// Configurational force on atoms corresponding to Gaussian generator. Generator
// is discretized using linear FE shape functions. Configurational force on
// nodes due to linear FE shape functions precomputed
  ...
  bool allowGaussianOverlapOnAtoms)
{
  ...
  vertex_touched.resize(
    d_dofHandlerForceElectro.get_triangulation().n_vertices(), false);
  ...
  if (d_dftParams.verbosity >= 1)
    {
      pcout << " Maximum absolute force atom id: " << maxForceAtomId
            ...
            << "," << -d_globalAtomsForces[3 * maxForceAtomId + 1] << ","
            << -d_globalAtomsForces[3 * maxForceAtomId + 2] << std::endl;
      pcout << " Sum of absolute value of all force components over all atoms: "
