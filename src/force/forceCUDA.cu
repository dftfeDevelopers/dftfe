...
namespace dftfe
{
  namespace forceCUDA
  {
        ...
        for (unsigned int index = globalThreadId; index < numberEntries;
             index += blockDim.x * gridDim.x)
          {
            ...
            nlpContractionContribution[index] =
              ...
              psiQuadValuesNLP[nonTrivialIdToElemIdMap[pseudoWfcId] *
                                 numQuadsNLP * numPsi +
                               quadId * numPsi + wfcId] *
    ...
    void
    computeNonLocalProjectorKetTimesPsiTimesVH(
      ...
      double *              projectorKetTimesPsiTimesVH)
    {
      ...
      operatorMatrix.computeNonLocalProjectorKetTimesXTimesV(
        cudaFlattenedArrayBlock.begin(), projectorKetTimesVector, BVec);
          ...
          if (currentBlockSize > 0)
            {
              ...
              cublasDgemmStridedBatched(
                ...
                thrust::raw_pointer_cast(
                  &(operatorMatrix
                      .getInverseJacobiansNLP())[startingId * numQuadsNLP * 3 *
                                                 3]),
                3,
                3 * 3,
                &scalarCoeffBeta,
              ...
              cublasDgemmStridedBatched(
                ...
                &scalarCoeffBeta,
                thrust::raw_pointer_cast(
                  &gradPsiQuadsNLPFlatD[startingId * numQuadsNLP * 3 * BVec]),
                BVec,
                strideCNLPGrad,
                currentBlockSize);
      ...
      for (int iblocknlp = 0; iblocknlp < (numberBlocksNlp + 1); iblocknlp++)
        {
          ...
          if (currentBlockSizeNlp > 0)
            {
              nlpPsiContractionCUDAKernel<<<(numPsi + 255) / 256 * numQuadsNLP *
                                              3 * currentBlockSizeNlp,
                                            256>>>(
                numPsi,
                numQuadsNLP * 3,
                currentBlockSizeNlp,
      ...
      if (isPsp)
        {
          ...
          operatorMatrix.computeNonLocalProjectorKetTimesXTimesV(
            cudaFlattenedArrayBlock.begin(), projectorKetTimesVectorD, numPsi);
      ...
      const bool      isPsp)
    {
      ...
      thrust::device_vector<double> psiQuadsFlatD(cellsBlockSize * numQuads *
                                                    blockSize,
                                                  0.0);
      thrust::device_vector<double> gradPsiQuadsXFlatD(cellsBlockSize *
                                                         numQuads * blockSize,
                                                       0.0);
      thrust::device_vector<double> gradPsiQuadsYFlatD(cellsBlockSize *
                                                         numQuads * blockSize,
                                                       0.0);
      thrust::device_vector<double> gradPsiQuadsZFlatD(cellsBlockSize *
                                                         numQuads * blockSize,
                                                       0.0);
      thrust::device_vector<double> gradPsiQuadsNLPFlatD(
        numCells * numQuadsNLP * 3 * blockSize, 0.0);
