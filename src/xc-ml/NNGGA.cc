...
template<typename IN1,
	...
	void
		  ...
		  torch::jit::script::Module * model)
	  {
		...
		interleave(rho, rho+numPoints, modGradRho, modGradRho+numPoints, 1, 1, &rhoFloat[0]);
		auto options = torch::TensorOptions().dtype(torch::kFloat32).requires_grad(true);
		torch::Tensor rhoTensor = torch::from_blob(&rhoFloat[0], {numPoints,2}, options).clone();
	...
	void
		  ...
		  torch::jit::script::Module * model)
	  {
		...
		interleave(rho, rho+2*numPoints, modGradRhoTotal, modGradRhoTotal+numPoints, 2, 1, &rhoFloat[0]);
	...
	void dexcSpinUnpolarized(const double * rho,
		const double * modGradRho,
		const unsigned int numPoints,
		double * exc,
		double * dexc,
		torch::jit::script::Module * model)
	{
	  ...
	  auto vxcTensor = torch::autograd::grad({excTensor}, {rhoTensor}, /*grad_outputs=*/{grad_output}, /*create_graph=*/true)[0];
	  for(unsigned int i = 0; i < numPoints; ++i)
	  {
		exc[i] = static_cast<double>(excTensor[i][0].item<float>());
	...
	void dexcSpinPolarized(const double * rho,
		const double * modGradRhoTotal,
		const unsigned int numPoints,
		double * exc,
		double * dexc,
		torch::jit::script::Module * model)
	{
	  ...
	  {
		exc[i] = static_cast<double>(excTensor[i][0].item<float>());
		dexc[3*i+0] = static_cast<double>(vxcTensor[i][0].item<float>());
		dexc[3*i+1] = static_cast<double>(vxcTensor[i][1].item<float>());
		dexc[3*i+2] = static_cast<double>(vxcTensor[i][2].item<float>());
	  }
  ...
  void NNGGA::evaluateexc(const double * rho,
	  const double * modGradRhoTotal,
	  const unsigned int numPoints,
	  double * exc)	
  {
	...
	if(!d_isSpinPolarized)
	  excSpinUnpolarized(rho, modGradRhoTotal, numPoints, exc, d_model);
	else
	  excSpinPolarized(rho, modGradRhoTotal, numPoints, exc, d_model);
  }

  void NNGGA::evaluatevxc(const double * rho,
	  const double * modGradRhoTotal,
	  const unsigned int numPoints,
	  double * exc,
	  double * dexc)
  {
	...
	if(!d_isSpinPolarized)
	  dexcSpinUnpolarized(rho, modGradRhoTotal, numPoints, exc, dexc, d_model);
	else
	  dexcSpinPolarized(rho, modGradRhoTotal, numPoints, exc, dexc, d_model);
  }
