...
namespace dftfe {
			       ...
			       const std::vector<std::vector<double> > & domainBoundingVectors)
  {
    ...
    if (d_isParallelMesh)
      {
	d_triaPtr=&(dynamic_cast<parallel::distributed::Triangulation<3> & >(triangulation));
  ...
  void meshMovementClass::initIncrementField()
  {
    ...
    d_incrementalDisplacement = vectorType(d_locally_owned_dofs,
					   ghost_indices,
					   mpi_communicator);
  ...
  void meshMovementClass::moveSubdividedMesh()
  {
    //
