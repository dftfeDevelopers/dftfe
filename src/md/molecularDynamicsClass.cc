            ...
            for(int jatomtype = 0; jatomtype < atomTypesMasses.size(); ++jatomtype)
              {
                for (int iCharge = 0; iCharge < numberGlobalCharges; ++iCharge)
                  { 
                    ...
                    pcout << "Initialiseng Velocities of species no: "<<jatomtype<<" mass in amu: "<<Mass
...
//--------------------Choosing Ensemble ----------------------------------------------//
    ...
    template <unsigned int FEOrder, unsigned int FEOrderElectro>
    void    
    molecularDynamicsClass<FEOrder, FEOrderElectro>::RescaleVelocities(std::vector<double> &v,double &KE,std::vector<double> M, double Temperature)
    {
    ...
    template <unsigned int FEOrder, unsigned int FEOrderElectro>
    void
    molecularDynamicsClass<FEOrder, FEOrderElectro>::NoseHoverChains(std::vector<double> &v, std::vector<double> &v_e, std::vector<double> &e, std::vector<double> Q, double KE, double  Temperature)
    {
      double G1, G2, s;
