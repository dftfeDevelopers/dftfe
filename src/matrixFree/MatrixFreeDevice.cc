...
namespace dftfe
{
  namespace MatrixFreeInternal
  {
    ...
    {
      __shared__ T
        sharedConstrainingData[batchSize * nDofsPerDim * nDofsPerDim];
      ...
      unsigned int constrainedBucketSize =
        constrainedNodeOffset[blockIdx.x + 1] -
        constrainedNodeOffset[blockIdx.x];
                  ...
                  const unsigned int *__restrict__ map)
    {
      ...
      constexpr unsigned int qEven =
        nQuadPointsPerDim % 2 == 1 ? qOdd + 1 : qOdd;

      T *__restrict__ sharedP = reinterpret_cast<T *>(sharedMemory);
      T *__restrict__ sharedQ =
        &sharedP[batchSize * nQuadPointsPerDim * nQuadPointsPerDim *
                   nQuadPointsPerDim +
                 padding];
              ...
              for (unsigned int j = 0; j < qOdd; j++)
                {
                  ...
                  regQ[j + qOdd] +=
                    constN[j + k * qOdd + qOdd * pOdd] * regP[3];
                }
            }
          ...
          for (unsigned int k = 0; k < pOdd; k++)
            {
              ...
              regQ[1] =
                sharedQ[threadIdx.x + (nDofsPerDim - 1 - k) * batchSize +
                        i * batchSize * nDofsPerDim];
              ...
              for (unsigned int j = 0; j < qOdd; j++)
                {
                  ...
                  regP[j + qOdd] +=
                    constN[j + k * qOdd + qOdd * pOdd] * regQ[3];
                }
            }
