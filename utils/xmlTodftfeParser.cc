        ...
        if(cur_node != nullptr){
            ...
            if(!traversal.empty()){
                ...
                if(cur_node->type == XML_ELEMENT_NODE){
                    ...
                    }else if(xmlStrcmp(cur_node->name, (const xmlChar *)"core_charge") == 0){
                        std::string coreDensity_str(reinterpret_cast<char*>(xmlNodeGetContent(cur_node)));
                        coreCharge_present = true;
                        if (!coreCharge_present){
                            std::cerr << "Core charge header not present" << std::endl;
                            break;
                        }
                        std::string::size_type n = 0;
                        while((n = coreDensity_str.find( "   ", n)) != std::string::npos){
...
bool xmlTodftfeParser::outputData(const std::string & baseOutputPath)
{
	...
	for(auto i : coreDensity)
	  {
	    double di = atof(i.c_str());
	    cd_file << std::fixed << std::setprecision(14) << mesh[jc] << " " << di << std::endl;
	    jc += 1;
	  }
      }
