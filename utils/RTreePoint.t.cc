...
namespace dftfe
{
  namespace utils
  {
                    ...
                    unsigned int nNearestNeighbours)
    {
            ...
            std::vector<std::pair<BG::model::point<double, dim, BG::cs::cartesian>, size_type>> result;

            result.resize(0);
            d_rtreePtr->query(BGI::nearest(bgInputPoint,nNearestNeighbours),std::back_inserter(result));
