...
namespace dftfe
{
                                  ...
                                  const bool         _useDevice)
  {
    ...
    prm.enter_subsection("Finite element mesh parameters");
    {
      ...
      finiteElementPolynomialOrderRhoNodal = prm.get_integer("POLYNOMIAL ORDER DENSITY NODAL") == 0 ?
        (finiteElementPolynomialOrder+2 > finiteElementPolynomialOrderElectrostatics ?
         finiteElementPolynomialOrder+2 : finiteElementPolynomialOrderElectrostatics) :
  ...
  !defined(DFTFE_WITH_DEVICE_AWARE_MPI)
    ...
    if (reproducible_output)
      ...
      {
        densityQuadratureRule = XCType.substr(0, 4) == "MGGA" ? 16: finiteElementPolynomialOrderRhoNodal+1;
      }
