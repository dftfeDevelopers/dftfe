...
namespace dftfe
{
 namespace dftUtils
 {
  QuadDataCompositeWrite::QuadDataCompositeWrite(const std::vector<double> &vals):
    d_vals(vals),
    d_charspernum(20)
  {
  ...
  void QuadDataCompositeWrite::getCharArray(char *data) {
  ...
  void QuadDataCompositeWrite::getMPIDataType(MPI_Datatype *mpi_datatype) {
  ...
  int QuadDataCompositeWrite::getNumberCharsPerCompositeData() {
