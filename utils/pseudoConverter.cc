...
namespace dftfe
{
  namespace pseudoUtils
  {
      ...
      while(input_file >> z >> toParse)
	{
	  ...
	  if(isupf(toParse))
	      ...
	      else
		{
		  ...
		  if(dftParameters::verbosity >= 1)
		    {
		      if(nlccFlag > 0)
			pcout<< " Reading Pseudopotential File: "<<toParse<<", with atomic number: "<< z<<", and has data for nonlinear core-correction"<<std::endl;
		      else
			pcout<< " Reading Pseudopotential File: "<<toParse<<", with atomic number: "<< z<<", and has no nonlinear core-correction"<<std::endl;
		    }
      ...
      //pcout<<"Few atoms have pseudopotentials with nonlinear core correction"<<std::endl;
