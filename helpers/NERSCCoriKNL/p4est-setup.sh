...
mkdir -p "$BUILD_FAST"
cd "$BUILD_FAST"
"$SRCDIR/configure" CC=cc CXX=CC FC=ftn F77=ftn --enable-mpi --enable-shared\
...
mkdir -p "$BUILD_DEBUG"
cd "$BUILD_DEBUG"
"$SRCDIR/configure" CC=cc CXX=CC FC=ftn F77=ftn --enable-debug --enable-mpi --enable-shared\
        --disable-vtk-binary --without-blas --enable-openmp=-qopenmp \
